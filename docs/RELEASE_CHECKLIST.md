...
4. From the GitHub Actions **Release** workflow on `main`, run the workflow with tag `v<build.gradle version>`.
5. Confirm the GitHub Release contains both the reobfuscated jar and its `.sha256` checksum.
6. Keep subsequent development on feature branches targeting `dev/core-industrial`.
