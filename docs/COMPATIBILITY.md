...
Assembler recipes use `justguithings:assembling` with one to four item inputs. Each input is a Minecraft `Ingredient`, a positive `count`, and an optional explicit `slot` from 0 through 3; omitted slots fall back to sequential order for simple datapacks. Slot 0 is required as the primary/provider input, slot IDs must be unique, and every unspecified machine slot must remain empty for deterministic matching. This explicit slot contract lets Forge `IItemHandler` automation target stable ingredient positions instead of guessing between recipes that share materials. Results use the same `RecipeOutput` abstraction as other JGT processing recipes, including tag-based provider resolution.
