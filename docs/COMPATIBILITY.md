...
Recipe serializers also reject input batches that cannot fit their machine's physical storage. Industrial Assembler ingredient counts and Industrial Mixer item counts are limited to 1-64 per input slot. Mixer fluid amounts are limited to 1-8,000 mB, Industrial Washer fluid amounts to 1-4,000 mB, and Fluid Generator fuel batches to 1-16,000 mB. These are capacity guards rather than balance limits: valid existing recipes are unchanged, while datapacks that describe an operation the target machine can never physically hold fail during recipe loading instead of remaining permanently unprocessable. Bundled JGT recipes are additionally checked by the Gradle `validateRecipePhysicalLimits` task during `processResources` and `check`, so repository recipes that exceed these capacities fail CI before packaging. Processing-time and FE/t scaling use widened arithmetic and saturate at `Integer.MAX_VALUE` before returning machine-facing `int` values, so extreme datapack values cannot wrap into artificially short or cheap operations. The Resistive Furnace Power Coil uses the same overflow-safe scaling contract.

Processing recipe serializers also reject base `energy_per_tick` values that can never fit the target machine's maximum physical energy capacity: Crusher 500,000 FE/t with four Buffer modules, Stamping Press 100,000, Industrial Mixer 150,000, Industrial Washer 120,000, Industrial Sawmill 80,000, Industrial Separator 80,000, Resistive Furnace 120,000, Rod Mill 140,000, Wire Mill 130,000, and Industrial Assembler 160,000. These limits apply to the recipe's base FE/t only; optional Speed, Batch, Efficiency and Power Coil effects remain runtime tradeoffs rather than datapack validation rules. The same bounds are enforced during recipe network sync and by `validateRecipePhysicalLimits` for bundled recipes. Network decoding also reapplies the same structural constraints used by JSON loading for ingredient presence and counted inputs; invalid Separator output counts abort decoding before any unbounded result loop can run.
...
When a configurable face changes mode at runtime, JGT replaces that face's owned sided `LazyOptional` views, invalidates the previous handles, and notifies block neighbors on the server. Standard external automation can therefore drop cached capability references and rediscover newly enabled/disabled I/O without requiring a block replacement. Persisted side configuration is also applied before machine-specific sided capability wrappers are rebuilt during NBT load, so restored `FLUID_INPUT`/`FLUID_OUTPUT` faces keep the correct fill-only or drain-only behavior after chunk/world reload.
...
The Auto Crafter automates the standard Minecraft `IRecipeType.CRAFTING` recipe pool instead of introducing another JGT processing serializer. A player arranges one valid 3x3 crafting recipe in the machine and locks it. The lock stores the concrete recipe ID plus the exact item/NBT sample assigned to each grid slot. Locking preserves Minecraft's normal crafting-recipe resolution; after the lock is created, processing resolves that stored recipe ID directly and revalidates it against the exact template. This prevents overlapping crafting recipes from making an existing lock switch or stall merely because recipe iteration order changes, while avoiding a separate JGT precedence policy for the vanilla/mod crafting pool.
