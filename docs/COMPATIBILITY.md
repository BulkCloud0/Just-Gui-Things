...
The Configurator stores its own explicit edit mode in item NBT. Existing and untagged Configurators default to `Side I/O`. Right-clicking in the air switches between `Side I/O`, `Machine Redstone`, and `Conduit Connection`; clicking a configurable face applies only the selected mode. `Side I/O` cycles the supported face mode on machines and conduits. `Machine Redstone` cycles operation control only on active machines that support it and never changes conduit or storage I/O. `Conduit Connection` applies only to a face touching another conduit of the same network type and toggles that physical/topological link on both sides. Shift-right-click remains read-only inspection: on machines, item pipes, fluid pipes and energy cables it reports the current localized configuration without changing side, redstone, or conduit-link state. Disabled conduit faces remain inspectable for diagnostics. Face directions are also localized through shared translatable components, so conduit/machine feedback no longer exposes raw NORTH/SOUTH/EAST/WEST/UP/DOWN enum names. Conduit inspection additionally reports Connected/Disconnected from the current multipart blockstate arm. This is a visual/physical connection indicator only; it does not imply that routing, redstone or filters currently allow transfer. While the Configurator is held and no GUI is open, the currently targeted configurable face receives a client-only world-space outline derived from its current side mode. The overlay reads the local TileEntity state only and adds no packets, NBT or blockstate properties. World overlays and machine side-config screens share a single client-side color palette so the same mode keeps the same visual identity. The HUD also shows the same localized target description used by Shift-inspection while the Configurator is aimed at a supported face, so I/O direction and resource type remain readable without relying on color alone. Compact machine side-config direction/mode codes are localized as well; the grid measures translated tokens before laying out columns so wider locale-specific abbreviations do not overlap.

## Automated Assembler

The Automated Assembler closes the manual-crafting gap after the metal-processing chain. It does not define a JGT-specific assembly recipe type. Instead, the machine resolves normal `IRecipeType.CRAFTING` recipes through Minecraft's live `RecipeManager`, so vanilla recipes, datapack replacements and compatible modded shaped/shapeless recipes remain the source of truth.

The player lays out a valid 3x3 crafting grid and explicitly locks it from the machine GUI. Locking stores the selected recipe ID plus one-count copies of the configured input stacks as a slot template. The physical inputs are still normal inventory stacks and are consumed during crafting; the stored template is only routing/configuration metadata. Once locked, each of the nine input slots accepts only the exact item/NBT variant configured for that position and exposes a one-item slot limit. This makes generic sided `IItemHandler` insertion distribute repeated ingredients across all required grid positions instead of filling the first matching slot to a full stack before moving on.

The machine exposes nine input slots, one output slot, and Speed/Efficiency module slots through the existing machine core. Input/output side modes and redstone operation control therefore follow the same Forge capability contracts as the other JGT machines. Clearing the recipe lock preserves all physical inventory contents and only removes the saved template/recipe selection.

Assembler v1 intentionally rejects recipes whose `getRemainingItems()` result contains a non-empty stack. This avoids silently losing or trapping reusable crafting containers such as buckets/tools. Supporting crafting remainders later requires a deliberate return/output policy rather than special-casing individual recipes.
