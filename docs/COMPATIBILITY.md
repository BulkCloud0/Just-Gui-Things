...
Assembler recipes use `justguithings:assembling` with one to four ordered item inputs. Each input is a Minecraft `Ingredient` plus a positive `count`, so datapacks can assemble components from shared Forge tags without Java integration. Inputs map to the Assembler's four automation slots; unused trailing slots must remain empty for deterministic matching. Results use the same `RecipeOutput` abstraction as other JGT processing recipes, including tag-based provider resolution.
