...
public class IndustrialWasherScreen extends ContainerScreen<IndustrialWasherContainer> {
    ...
    public IndustrialWasherScreen(IndustrialWasherContainer menu, PlayerInventory inventory, ITextComponent title) {
    ...
    protected void renderBg(MatrixStack matrixStack, float partialTicks, int mouseX, int mouseY) {
        ...
        if (progressWidth > 0) {
            AbstractGui.fill(matrixStack, left + 70, top + 39,
                    left + 70 + progressWidth, top + 49, PROGRESS_COLOR);
        }

        int energyHeight = menu.getEnergyScaled(48);
        AbstractGui.fill(matrixStack, left + 147, top + 24, left + 158, top + 74, ENERGY_BG_COLOR);
        if (energyHeight > 0) {
            AbstractGui.fill(matrixStack, left + 149, top + 72 - energyHeight,
                    left + 156, top + 72, ENERGY_COLOR);
        }

        int fluidHeight = menu.getFluidScaled(48);
        AbstractGui.fill(matrixStack, left + 160, top + 24, left + 171, top + 74, FLUID_BG_COLOR);
        if (fluidHeight > 0) {
            AbstractGui.fill(matrixStack, left + 162, top + 72 - fluidHeight,
                    left + 169, top + 72, FLUID_COLOR);
        }
    }
    ...
    protected void renderLabels(MatrixStack matrixStack, int mouseX, int mouseY) {
        ...
        font.draw(matrixStack,
                "FE: " + menu.getEnergyStored() + " / " + IndustrialWasherTileEntity.CAPACITY,
                78.0F, 6.0F, TEXT_COLOR);
        font.draw(matrixStack, menu.getCurrentEnergyPerTick() + " FE/t",
