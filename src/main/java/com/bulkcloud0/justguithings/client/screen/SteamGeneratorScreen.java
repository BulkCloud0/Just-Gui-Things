...
public class SteamGeneratorScreen extends ContainerScreen<SteamGeneratorContainer> {
    ...
    private static final int WATER = 0xFF397FE6;
    private static final int BURN = 0xFFFF9E3D;
    private static final int TEXT = 0xFFE5E9ED;

    public SteamGeneratorScreen(SteamGeneratorContainer menu, PlayerInventory inventory, ITextComponent title) {
    ...
    protected void renderBg(MatrixStack matrixStack, float partialTicks, int mouseX, int mouseY) {
        ...
        int waterHeight = menu.getFluidScaled(48);
        AbstractGui.fill(matrixStack, left + 18, top + 24, left + 29, top + 74, BAR_BG);
        if (waterHeight > 0) {
            AbstractGui.fill(matrixStack, left + 20, top + 72 - waterHeight, left + 27, top + 72, WATER);
        }

        int burnHeight = menu.getBurnScaled(18);
        AbstractGui.fill(matrixStack, left + 80, top + 34, left + 88, top + 54, BAR_BG);
        if (burnHeight > 0) {
            AbstractGui.fill(matrixStack, left + 81, top + 53 - burnHeight, left + 87, top + 53, BURN);
        }
    ...
    protected void renderLabels(MatrixStack matrixStack, int mouseX, int mouseY) {
        ...
        font.draw(matrixStack,
                I18n.get("screen.justguithings.steam_generator.water", menu.getFluidAmount()),
                8.0F, 55.0F, TEXT);
        font.draw(matrixStack,
                "FE: " + menu.getEnergyStored() + " / " + SteamGeneratorTileEntity.ENERGY_CAPACITY,
                82.0F, 6.0F, TEXT);
        font.draw(matrixStack,
