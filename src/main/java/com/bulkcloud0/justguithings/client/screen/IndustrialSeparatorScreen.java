package com.bulkcloud0.justguithings.client.screen;

import com.bulkcloud0.justguithings.world.container.IndustrialSeparatorContainer;
import com.bulkcloud0.justguithings.world.tile.IndustrialSeparatorTileEntity;
import com.mojang.blaze3d.matrix.MatrixStack;
import net.minecraft.client.gui.AbstractGui;
import net.minecraft.client.gui.screen.inventory.ContainerScreen;
import net.minecraft.entity.player.PlayerInventory;
import net.minecraft.util.text.ITextComponent;

public class IndustrialSeparatorScreen extends ContainerScreen<IndustrialSeparatorContainer> {
    private static final int PANEL_COLOR = 0xFF20262E;
    private static final int INNER_COLOR = 0xFF313942;
    private static final int SLOT_COLOR = 0xFF11161C;
    private static final int SLOT_BORDER_COLOR = 0xFF697582;
    private static final int ENERGY_BG_COLOR = 0xFF101419;
    private static final int ENERGY_COLOR = 0xFF2FC3D8;
    private static final int PROGRESS_BG_COLOR = 0xFF101419;
    private static final int PROGRESS_COLOR = 0xFF8FA8B8;
    private static final int TEXT_COLOR = 0xFFE5E9ED;

    public IndustrialSeparatorScreen(IndustrialSeparatorContainer menu, PlayerInventory inventory, ITextComponent title) {
        super(menu, inventory, title);
        imageWidth = 176;
        imageHeight = 166;
        inventoryLabelY = 72;
    }

    @Override
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        renderBackground(matrixStack);
        super.render(matrixStack, mouseX, mouseY, partialTicks);
        renderTooltip(matrixStack, mouseX, mouseY);
    }

    @Override
    protected void renderBg(MatrixStack matrixStack, float partialTicks, int mouseX, int mouseY) {
        int left = leftPos;
        int top = topPos;
        AbstractGui.fill(matrixStack, left, top, left + imageWidth, top + imageHeight, PANEL_COLOR);
        AbstractGui.fill(matrixStack, left + 5, top + 18, left + 171, top + 76, INNER_COLOR);
        AbstractGui.fill(matrixStack, left + 5, top + 80, left + 171, top + 161, INNER_COLOR);

        drawSlot(matrixStack, left + 35, top + 34);
        drawSlot(matrixStack, left + 107, top + 34);
        drawSlot(matrixStack, left + 125, top + 34);
        drawSlot(matrixStack, left + 143, top + 34);
        drawSlot(matrixStack, left + 71, top + 55);
        drawSlot(matrixStack, left + 89, top + 55);

        for (int row = 0; row < 3; row++) {
            for (int column = 0; column < 9; column++) {
                drawSlot(matrixStack, left + 7 + column * 18, top + 83 + row * 18);
            }
        }
        for (int column = 0; column < 9; column++) {
            drawSlot(matrixStack, left + 7 + column * 18, top + 141);
        }

        AbstractGui.fill(matrixStack, left + 62, top + 38, left + 102, top + 50, PROGRESS_BG_COLOR);
        int progressWidth = menu.getProgressScaled(38);
        if (progressWidth > 0) {
            AbstractGui.fill(matrixStack, left + 63, top + 39,
                    left + 63 + progressWidth, top + 49, PROGRESS_COLOR);
        }

        int energyHeight = menu.getEnergyScaled(48);
        AbstractGui.fill(matrixStack, left + 162, top + 24, left + 173, top + 74, ENERGY_BG_COLOR);
        if (energyHeight > 0) {
            AbstractGui.fill(matrixStack, left + 164, top + 72 - energyHeight,
                    left + 171, top + 72, ENERGY_COLOR);
        }
    }

    private void drawSlot(MatrixStack matrixStack, int x, int y) {
        AbstractGui.fill(matrixStack, x, y, x + 18, y + 18, SLOT_BORDER_COLOR);
        AbstractGui.fill(matrixStack, x + 1, y + 1, x + 17, y + 17, SLOT_COLOR);
    }

    @Override
    protected void renderLabels(MatrixStack matrixStack, int mouseX, int mouseY) {
        font.draw(matrixStack, title, 8.0F, 6.0F, TEXT_COLOR);
        SideConfigRenderer.drawHorizontal(matrixStack, font, 8.0F, 19.0F, menu::getSideMode);
        font.draw(matrixStack, inventory.getDisplayName(), 8.0F, inventoryLabelY, TEXT_COLOR);
        font.draw(matrixStack,
                "FE: " + menu.getEnergyStored() + " / " + IndustrialSeparatorTileEntity.CAPACITY,
                80.0F, 6.0F, TEXT_COLOR);
        font.draw(matrixStack,
                "S" + menu.getSpeedUpgradeCount() + " E" + menu.getEfficiencyUpgradeCount()
                        + " | " + menu.getCurrentEnergyPerTick() + " FE/t",
                8.0F, 60.0F, TEXT_COLOR);
    }
}
