...
public class CrusherBlock extends Block {
    ...
    public void onRemove(BlockState state, World world, BlockPos pos, BlockState newState, boolean isMoving) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (tile instanceof CrusherTileEntity) {
                MachineInventoryDropHelper.dropContents(world, pos, ((CrusherTileEntity) tile).getInventory());
            }
        }
    ...
    public ActionResultType use(BlockState state, World world, BlockPos pos, PlayerEntity player,
        ...
        if (tile instanceof CrusherTileEntity && held.getItem() == ModItems.CONFIGURATOR.get()) {
            if (!world.isClientSide) {
                CrusherTileEntity crusher = (CrusherTileEntity) tile;
                MachineSideMode mode = crusher.cycleSideMode(hit.getDirection());
