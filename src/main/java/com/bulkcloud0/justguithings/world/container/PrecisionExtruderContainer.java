...
public class PrecisionExtruderContainer extends Container {
    ...
    private final PrecisionExtruderTileEntity tileEntity;
    private final IIntArray data;
    private final SideConfigContainerData sideData;

    public PrecisionExtruderContainer(int windowId, PlayerInventory playerInventory, PacketBuffer buffer) {
        this(windowId, playerInventory, getTileEntity(playerInventory, buffer));
    }

    public PrecisionExtruderContainer(int windowId, PlayerInventory playerInventory, PrecisionExtruderTileEntity tileEntity) {
        super(ModContainers.PRECISION_EXTRUDER.get(), windowId);
    ...
    private static PrecisionExtruderTileEntity getTileEntity(PlayerInventory playerInventory, PacketBuffer buffer) {
        BlockPos pos = buffer.readBlockPos();
        TileEntity tile = playerInventory.player.level.getBlockEntity(pos);
        if (!(tile instanceof PrecisionExtruderTileEntity)) {
            throw new IllegalStateException("Precision Extruder tile entity not found at " + pos);
        }
        return (PrecisionExtruderTileEntity) tile;
    }
    ...
    public int getEnergyScaled(int pixels) {
        return (int) ((long) getEnergyStored() * pixels / PrecisionExtruderTileEntity.CAPACITY);
    }
