...
public class AssemblerContainer extends Container {
    ...
    public AssemblerContainer(int windowId, PlayerInventory playerInventory, PacketBuffer buffer) {
        this(windowId, playerInventory, getTileEntity(playerInventory, buffer));
    }

    public AssemblerContainer(int windowId, PlayerInventory playerInventory, AssemblerTileEntity tileEntity) {
        super(ModContainers.ASSEMBLER.get(), windowId);
        this.tileEntity = tileEntity;
        this.data = tileEntity.getDataAccess();
        this.sideData = new SideConfigContainerData(tileEntity, tileEntity::getSideMode);

        this.addSlot(new SlotItemHandler(tileEntity.getInventory(), 0, 26, 28));
        this.addSlot(new SlotItemHandler(tileEntity.getInventory(), 1, 44, 28));
        this.addSlot(new SlotItemHandler(tileEntity.getInventory(), 2, 26, 46));
        this.addSlot(new SlotItemHandler(tileEntity.getInventory(), 3, 44, 46));
        this.addSlot(new SlotItemHandler(tileEntity.getInventory(), AssemblerTileEntity.OUTPUT_SLOT, 116, 37) {
            @Override
    ...
    private static AssemblerTileEntity getTileEntity(PlayerInventory playerInventory, PacketBuffer buffer) {
        BlockPos pos = buffer.readBlockPos();
        TileEntity tile = playerInventory.player.level.getBlockEntity(pos);
        if (!(tile instanceof AssemblerTileEntity)) {
            throw new IllegalStateException("Assembler tile entity not found at " + pos);
        }
        return (AssemblerTileEntity) tile;
    }
    ...
    private boolean moveToInput(ItemStack stack) {
        for (int input = 0; input < AssemblerTileEntity.INPUT_SLOTS; input++) {
            if (tileEntity.canAcceptInput(input, stack)
                    && this.moveItemStackTo(stack, input, input + 1, false)) {
                return true;
            }
        }
        return false;
    ...
    public int getEnergyScaled(int pixels) {
        return (int) ((long) getEnergyStored() * pixels / AssemblerTileEntity.CAPACITY);
    }
