...
public class RodMillContainer extends Container {
    ...
    private final RodMillTileEntity tileEntity;
    private final IIntArray data;
    private final SideConfigContainerData sideData;

    public RodMillContainer(int windowId, PlayerInventory playerInventory, PacketBuffer buffer) {
        this(windowId, playerInventory, getTileEntity(playerInventory, buffer));
    }

    public RodMillContainer(int windowId, PlayerInventory playerInventory, RodMillTileEntity tileEntity) {
        super(ModContainers.ROD_MILL.get(), windowId);
    ...
    private static RodMillTileEntity getTileEntity(PlayerInventory playerInventory, PacketBuffer buffer) {
        BlockPos pos = buffer.readBlockPos();
        TileEntity tile = playerInventory.player.level.getBlockEntity(pos);
        if (!(tile instanceof RodMillTileEntity)) {
            throw new IllegalStateException("Rod Mill tile entity not found at " + pos);
        }
        return (RodMillTileEntity) tile;
    }
    ...
    public int getEnergyScaled(int pixels) {
        return (int) ((long) getEnergyStored() * pixels / RodMillTileEntity.CAPACITY);
    }
