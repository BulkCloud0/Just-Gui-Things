...
public class WireMillContainer extends Container {
    ...
    private final WireMillTileEntity tileEntity;
    private final IIntArray data;
    private final SideConfigContainerData sideData;

    public WireMillContainer(int windowId, PlayerInventory playerInventory, PacketBuffer buffer) {
        this(windowId, playerInventory, getTileEntity(playerInventory, buffer));
    }

    public WireMillContainer(int windowId, PlayerInventory playerInventory, WireMillTileEntity tileEntity) {
        super(ModContainers.WIRE_MILL.get(), windowId);
    ...
    private static WireMillTileEntity getTileEntity(PlayerInventory playerInventory, PacketBuffer buffer) {
        BlockPos pos = buffer.readBlockPos();
        TileEntity tile = playerInventory.player.level.getBlockEntity(pos);
        if (!(tile instanceof WireMillTileEntity)) {
            throw new IllegalStateException("Wire Mill tile entity not found at " + pos);
        }
        return (WireMillTileEntity) tile;
    }
    ...
    public int getEnergyScaled(int pixels) {
        return (int) ((long) getEnergyStored() * pixels / WireMillTileEntity.CAPACITY);
    }
